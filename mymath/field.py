...
from scipy.interpolate import RegularGridInterpolator as regular_interp, \
...
class _AbsScalarFieldSymCylin(object):
  ...
  #     return ScalarFieldInSymCylinderRegularCoord(r_crds=self.r_crds,
  #     z_crds=self.z_crds,
  #                                                 scalar=new_scalar)


class ScalarFieldSymCylinRegularGrid(_AbsScalarFieldSymCylin):
  ...
  def __init__(self, *, r_crds, z_crds, scalar=None):
    ...
    ----------
    r_crds z_crds
      one dim array. increasing or decreasing order.
    scalar
      two dim matrix
    """
  ...
  def set_scalar(self, scalarValue):
    ...
    self.interp_f = regular_interp((self.r_crds, self.z_crds),
                                   self.scalar, method="cubic")

  def getMultiPointValues(self, pointArry):
    return self.interp_f((pointArry.rho, pointArry.z))


class ScalarFieldSymCylinUnstructGrid(_AbsScalarFieldSymCylin):
  ...
  def set_scalar(self, scalarValue):
    self.scalar = scalarValue
    self.interp_f = unstruct_interp((self.r_crds, self.z_crds),
                                    self.scalar)
